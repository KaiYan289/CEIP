...
class ManipulateEnv(hand_env.HandEnv):
    def __init__(
        ...
        assert self.target_rotation in ["ignore", "fixed", "xyz", "z", "parallel", "assign"]
    ...
    def compute_reward(self, achieved_goal, goal, info):
        # print("computing reward...", self.reward_type)
        if self.reward_type == "sparse":
            success = self._is_success(achieved_goal, goal).astype(np.float32)
            return success - 1.0
        else:
            d_pos, d_rot = self._goal_distance(achieved_goal, goal)
            # print("d_pos:", d_pos, "d_rot:", d_rot, "reward:", -(10.0 * d_pos + d_rot))
    ...
    def _reset_sim(self):
        ...
        if self.randomize_initial_rotation:
            if self.target_rotation == "z":
                ...
                initial_quat = rotations.quat_mul(initial_quat, offset_quat)
            elif self.target_rotation in ["xyz", "ignore", "assign"]:
                angle = self.np_random.uniform(-np.pi, np.pi)
    ...
    def _sample_goal(self, direction=None):
        print("Warning: This Environment is modified.")
        ...
        elif self.target_rotation.find('assign') != -1:  # assuming on the basis of xyz; we use fixed/ignore for position and assigned for rotation
            if direction is not None:
                pi = math.pi
                angle = direction * pi / 4
                axis = np.array([math.sin(-pi/4 + direction * pi/4), math.sin(3 * pi / 4 + direction * pi/4), math.sin(direction * pi/4)])
                target_quat = quat_from_angle_and_axis(angle, axis)
            else: # degenerate to xyz
                print("degenerating to xyz!")
                angle = self.np_random.uniform(-np.pi, np.pi)
                axis = self.np_random.uniform(-1.0, 1.0, size=3)
                target_quat = quat_from_angle_and_axis(angle, axis)
        else:
...
"""
# manipulate_egg_ignoreposition_assignrotation
class HandEggEnv(ManipulateEnv, utils.EzPickle):
    def __init__(
        self, target_position="random", target_rotation="xyz", reward_type="sparse"
        ):
        target_position="ignore"
        target_rotation="assign"
        reward_type="dense"
        utils.EzPickle.__init__(self, target_position, target_rotation, reward_type)
        ManipulateEnv.__init__(
            self,
